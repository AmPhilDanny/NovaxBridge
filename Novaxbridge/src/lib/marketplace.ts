...
export async function sendConnectionRequest(followingId: string): Promise<Connection> {
  ...
  const { data: existing } = await supabase
    .from('connections')
    .select('id, status')
    .or(`and(follower_id.eq.${userId},following_id.eq.${followingId}),and(follower_id.eq.${followingId},following_id.eq.${userId})`)
    .maybeSingle();

  if (existing?.status === 'accepted') throw new Error('Already connected with this user');
  if (existing?.status === 'pending') throw new Error('Connection request already sent');
...
export async function removeConnection(peerUserId: string): Promise<void> {
  ...
  const { data: conns, error: findErr } = await supabase
    .from('connections')
    .select('id')
    .or(`and(follower_id.eq.${userId},following_id.eq.${peerUserId}),and(follower_id.eq.${peerUserId},following_id.eq.${userId})`);

  if (findErr) throw new Error(findErr.message);
  if (!conns || conns.length === 0) throw new Error('Connection not found');

  const ids = conns.map((c) => c.id);
  const { error: delErr } = await supabase.from('connections').delete().in('id', ids);
